...
```bash
ddev exec --dir packages/<theme-name> npm install --save-dev tailwindcss postcss @tailwindcss/postcss postcss-cli cssnano
```
...
```bash
ddev exec --dir packages/<theme-name> npm run build        # Production build
ddev exec --dir packages/<theme-name> npm run dev          # Watch mode during development
ddev exec --dir packages/<theme-name> npm run test:visual  # Run VRT suite
```
...
- [ ] `ddev exec --dir packages/<theme-name> npm run build` completes without errors
